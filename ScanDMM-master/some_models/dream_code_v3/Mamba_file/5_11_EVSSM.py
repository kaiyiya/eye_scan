...
class EVS(nn.Module):
    ...
    def forward(self, x):
        if self.att:
            ...
            if self.idx % 2 == 1:
                x = torch.flip(x, dims=(-2, -1)).contiguous() # 翻转
            if self.idx % 2 == 0:
                x = torch.transpose(x, dim0=-2, dim1=-1).contiguous() # 转置
#            if self.idx % 4 == 3:
#                x = torch.flip(x, dims=(-2, -1)).contiguous()
#            if self.idx % 4 == 0:
#                x = torch.transpose(x, dim0=-2, dim1=-1).contiguous()

            x = self.grids(x) # (B,C,H,W)
...
if __name__ == '__main__':
    # (B,C,H,W)
    x1 = torch.randn(1,64,16,16).to(device)
    B,C,H,W = x1.shape
    # idx: EVS当前所处整个网络的第idx层
    Model = EVS(dim=C, ffn_expansion_factor=2, bias=True, att=True, idx=3, patch=H*W).to(device)
