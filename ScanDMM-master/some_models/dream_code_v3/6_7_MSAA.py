...
if __name__ == '__main__':
    # (B,C,H,W)   B:batchsize; L:序列长度/图像patch序列; C:通道数量
    x1 = torch.randn(1,64,224,224).to(device)
    x2 = torch.randn(1, 64, 224, 224).to(device)
    x3 = torch.randn(1, 64, 224, 224).to(device)

    # in_channels: 尺度个数 * 通道;  out_channels:通道
    Model = MSAA(in_channels=3*64,out_channels=64).cuda()

    out = Model(x1,x2,x3) # (B,C,H,W)
    print(out.shape)