...
if __name__ == '__main__':
    ...
    X = torch.randn(1,8,1,64).to(device)
    ...
    # 通过卷积构造金字塔结构  (粗尺度构造模块)
    conv_layers = Conv_Construct(d_model=64, window_size=[2, 2, 2]).to(device)
    # 定义多头注意力机制
    Model = MultiHeadAttention(n_head=8, d_model=64, d_k=8, dropout=0., normalize_before=False, all_size=all_size).to(device)
